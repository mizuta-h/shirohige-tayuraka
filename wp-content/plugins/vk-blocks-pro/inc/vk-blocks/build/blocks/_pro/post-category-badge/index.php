...
/**
 * Get target taxonomy and term for single display
 *
 * @param WP_Post $post Post object.
 * @param string  $taxonomy Taxonomy slug.
 * @return array|false Array with 'taxonomy' and 'term' keys, or false if not found.
 */
function vk_blocks_get_single_display_term( $post, $taxonomy = '' ) {
	// 特定のタクソノミーが選択されている場合
	if ( $taxonomy ) {
...
function vk_blocks_post_category_badge_render_callback( $attributes, $content, $block ) {
	...
	$term_info = vk_blocks_get_single_display_term( $post, $taxonomy );
	if ( ! $term_info ) {
		return '';
	}
	...
	if ( class_exists( '\VektorInc\VK_Term_Color\VkTermColor' ) && method_exists( '\VektorInc\VK_Term_Color\VkTermColor', 'get_post_single_term_info' ) ) {
		$term_color_info = \VektorInc\VK_Term_Color\VkTermColor::get_post_single_term_info( $post, array( 'taxonomy' => $term_info['taxonomy'] ) );
	} else {
		return '';
	}
