...
class VK_Blocks_Blog_Card {
	...
	public static function vk_get_blog_card_data_from_url( $url ) {
		...
		// WordPressサイトの場合のみ、個別の記事レベルで埋め込み可能性を判定
		if ( static::is_wordpress_site( $body ) && ! $is_embeddable ) {
			$blog_card_data['cannot_embed'] = true;
		} elseif ( ! empty( $url ) && empty( $blog_card_data['title'] ) && empty( $blog_card_data['featured_image'] ) && empty( $blog_card_data['excerpt'] ) && empty( $blog_card_data['site_logo'] ) && empty( $blog_card_data['site_title'] ) ) {
			// domain以外何もない場合はcannot_embedをtrueにする（OGタグが取れないだけの場合は許可）
			$blog_card_data['cannot_embed'] = true;
		}
	 ...
	 * @return boolean
	 */
	public static function is_wordpress_site( $body ) {
		// WordPressの特徴的な要素をチェック
		// 1. generatorメタタグ（最も信頼性が高い）
		if ( preg_match( '/<meta[^>]*name=["\']generator["\'][^>]*content=["\'][^"\']*WordPress[^"\']*["\'][^>]*>/i', $body ) ) {
			return true;
		}

		// 2. wp-content/wp-includes のリソース参照（href|src属性に限定）
		if ( preg_match( '/<(?:link|script|img)[^>]+(?:href|src)=["\'][^"\']*(?:wp-content|wp-includes)[^"\']*["\'][^>]*>/i', $body ) ) {
			return true;
		}

		// 3. WordPress API リンク（標準のAPIリンクの存在で判定）
